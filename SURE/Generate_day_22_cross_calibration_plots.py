...
hi = d.Peak.ix[22].ix[2].groupby(level =['Shot', 'Channel']).min()
hiu = hi.unstack(1)
hiu = hiu.fillna(0)
hiu = hiu[hiu[3] > -0.7]
'''
...
slope, intercept, r_value, p_value, error= stats.linregress(-ciu[1][0:150],-hiu[3][0:150]) #Gives calibration statistics for each plot
plt.plot([0, 0.5], [intercept, (slope /2) + intercept], "r-") #Plots a calibration line over the scatter plot
